/**
 * "The team behind X" panel on an initiative page (Figma 329:2122,
 ...
 * member portraits.
 *
 * The two columns are fixed-width and pushed apart (516 and 277 inside the
 * 80px-inset 1056 panel), not a flexible split — the heading is meant to break
 * after "заедно", which only happens at 516.
 */
...
export function InitiativeTeamPanel({
  ...
  return (
    <div className="bg-[rgba(21,21,21,0.05)] px-8 py-10 md:px-20 md:py-12">
      <div className="flex flex-col gap-10 md:flex-row md:items-end md:justify-between md:gap-12">
        <div className="flex flex-col items-start gap-8 md:w-[516px] md:max-w-full md:shrink">
          <h2 className="t-h02">{team.heading}</h2>
          <p className="t-body">{team.text}</p>
          <Button href={team.cta.href} variant="secondary" className="min-w-[304px]">
            {team.cta.label}
          </Button>
        </div>

        {members.length > 0 && (
          <div className="flex flex-col items-start gap-8 md:shrink-0 md:items-end md:pt-[90px]">
            {/* The credit carries its own line break — see creditLabel. */}
            <p className="t-caption whitespace-pre-line md:w-[277px] md:text-right">
              {team.creditLabel}
            </p>
