...
const field =
  "t-body w-full border-0 border-b border-border bg-transparent py-3 " +
  "focus:outline-none focus:border-b-2 focus:border-current";
          ...
          <div className="flex max-w-[1056px] flex-col gap-12">
            ...
            <form method="post" action="/api/partner" className="flex max-w-[540px] flex-col gap-8">
              ...
              {/* Honeypot: invisible to people, filled in by naive bots. The
                  API pretends success when it arrives non-empty. aria-hidden +
                  tabIndex so no real visitor — screen reader or keyboard —
                  ever lands in it. */}
              ...
              <label className="flex flex-col gap-2">
                ...
                {/* appearance-none + rounded-none strip the OS's own pill and
                    glyph so the select reads as another ruled field; the solid
                    triangle is the site's corner-mark family, not the UA's. */}
